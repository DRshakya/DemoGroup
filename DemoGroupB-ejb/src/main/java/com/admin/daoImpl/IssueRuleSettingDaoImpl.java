...
public class IssueRuleSettingDaoImpl extends StatusableDaoImpl<IssueRuleSetting> implements IssueRuleSettingDao{
    ...
    {
       StringBuilder stringBuilder = new StringBuilder("SELECT count(s.id) from IssueRuleSetting s where s.memberType=:memberType AND " +
                "s.status.statusDesc !=:statusDesc");
        Query query = getEntityManager().createQuery(stringBuilder.toString());
        System.out.println("+++++++++++++++++++++++++++"+issueRuleSettingDto.getSemester());
        query.setParameter("memberType",issueRuleSettingDto.getMemberType());
